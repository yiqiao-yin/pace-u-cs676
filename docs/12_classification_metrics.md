...
The three figures on this page are generated from scratch with numpy and
matplotlib by
[`tools/figures/make_metrics_figures.py`](https://github.com/yiqiao-yin/pace-u-cs676/blob/main/tools/figures/make_metrics_figures.py).
You are welcome to read it and reuse the code — see
[Reproducing the figures](#reproducing-the-figures-on-this-page) at the end.
...
- [The F1 score](#the-f1-score)
  - [Why the harmonic mean, and not the ordinary one](#why-the-harmonic-mean-and-not-the-ordinary-one)
...
- [Homework](#homework)
- [Reproducing the figures on this page](#reproducing-the-figures-on-this-page)
...
Every metric in this chapter is one question asked of these four numbers, and each
uses a different subset of them:

![Which confusion-matrix cells each metric uses](../pics/12_metrics_01.png)

The counts are identical in all four panels — they are the real output of
[homework 02](https://github.com/yiqiao-yin/pace-u-cs676/blob/main/notebooks/homework/02_logreg.py).
What changes is which cells the metric looks at. Accuracy uses all four.
Sensitivity ignores the entire bottom row. Precision ignores the entire left
column. That is the whole difference between them.
...
### Why the harmonic mean, and not the ordinary one
[Go back to TOC](#table-of-contents)

That is the *behaviour* we want, but it sounds like a lucky accident — as though we
went looking for a formula that punishes imbalance and happened to find one. It is
not an accident. The harmonic mean is the correct way to average *rates*, and the
cleanest way to see that has nothing to do with classifiers.

Suppose you drive to work and back. The distance each way is $d$. You drive out at
$x$ miles per hour and, in traffic, back at $y$. What was your average speed for the
round trip?

The tempting answer is $\frac{x + y}{2}$. It is wrong, and you can catch it without
any algebra: average speed is *total distance over total time*, so work out both.

$$
\text{ave speed}
= \frac{\text{distance}}{\text{time}}
= \frac{2d}{\dfrac{d}{x} + \dfrac{d}{y}}
= \frac{2d}{\dfrac{dy + dx}{xy}}
= \frac{2dxy}{d(y + x)}
= \frac{2xy}{x + y}
$$

The $d$ cancels — the distance never mattered. What is left is
$\frac{2xy}{x+y}$: **the harmonic mean**, and the same shape as the $F_1$ formula
above with $x$ and $y$ standing in for precision and recall.

![Averaging rates: the commute argument, and F1 across the precision-recall plane](../pics/12_metrics_03.png)

Put numbers on it. Out at 60 mph, back at 20 mph, 30 miles each way: half an hour
out, an hour and a half back, so 60 miles in 2 hours — **30 mph**. The arithmetic
mean says 40. It is not a worse estimate, it is an answer to a different question,
and the reason is visible in the times: you spend three times as long crawling at 20
as you do cruising at 60, so the slow leg deserves more weight, not equal weight.

**That is exactly the property $F_1$ needs.** Precision and recall are rates too, and
a model with one of them near zero has a real weakness that an average should not be
allowed to hide. The harmonic mean drags the result toward the smaller of the two —
it can never exceed it by much — while the arithmetic mean lets a strong component
paper over a weak one.

The right-hand panel above shows this over the whole plane. Both marked models have
an **arithmetic mean of 0.55**. The balanced one (precision 0.55, recall 0.55) scores
$F_1 = 0.55$; the lopsided one (precision 0.98, recall 0.12) scores **0.21**. The
second model finds one positive case in eight. It should not be allowed to look like
a coin flip, and under $F_1$ it does not.

### A worked example
[Go back to TOC](#table-of-contents)

Take the confusion matrix used throughout this chapter — the counts homework 2
produces on its validation split, $\mathrm{TP} = 139$, $\mathrm{FP} = 12$,
$\mathrm{FN} = 11$, $\mathrm{TN} = 138$:

$$
\text{Precision} = \frac{139}{139 + 12} = 0.920
\qquad
\text{Recall} = \frac{139}{139 + 11} = 0.927
$$

$$
F_1 = 2 \cdot \frac{0.920 \times 0.927}{0.920 + 0.927} = 0.924
$$

The two components are close, so $F_1$ lands between them and near the accuracy of
0.923 — on balanced data with balanced errors, these metrics agree, and reporting
$F_1$ tells you little that accuracy did not. That is worth seeing, because it makes
the opposite case sharper: **$F_1$ earns its keep precisely when the classes are
imbalanced.**

Consider a screen for a condition affecting 1% of patients, and a model that predicts
"healthy" for everyone. Out of 1000 patients it gets 990 right, so **accuracy is
99%** — and the model is worthless. Precision and recall are both 0, so **$F_1$ is
0**, which is the honest answer. Note *why* they differ: accuracy counts the 990 true
negatives in its numerator, and $F_1$ has no TN term at all. It is built entirely
from TP, FP, and FN, so a mountain of correctly-ignored negatives cannot inflate it.
...
![ROC curves and the score distributions behind them](../pics/12_metrics_02.png)

The left panel shows three classifiers on the same problem, differing only in how
well their scores separate the classes. The right panel shows where a curve comes
from: two score distributions, one threshold, and the two kinds of error it
creates. Sliding that threshold traces out the curve on the left — **every point on
an ROC curve is one choice of threshold.**

Notice that the shaded areas trade against each other. You cannot shrink the red
region without growing the blue one. You can only decide which you would rather
have.
...
for setup, and [DEADLINES.md](https://github.com/yiqiao-yin/pace-u-cs676/blob/main/DEADLINES.md) for the due date.

## Reproducing the figures on this page
[Go back to TOC](#table-of-contents)

Both figures above are drawn from scratch in about a hundred lines of numpy and
matplotlib — no plotting library beyond matplotlib, and nothing hidden:

```bash
pip install numpy matplotlib
python tools/figures/make_metrics_figures.py     # writes pics/12_metrics_0*.png
```

The source is
[`tools/figures/make_metrics_figures.py`](https://github.com/yiqiao-yin/pace-u-cs676/blob/main/tools/figures/make_metrics_figures.py).
You are welcome to copy it. Two parts are worth reading even if you never run it:

- `confusion_panel()` draws a confusion matrix as four rectangles and shades them by
  role. Any figure of this kind is just coloured boxes with numbers in them.
- `roc_from_scores()` computes an ROC curve **by definition** — sort the scores
  descending, take cumulative sums of the positives and negatives, and divide. Six
  lines, no library call. If the curve has ever felt like a black box, that function
  is the whole of it.

If you need plots for your own project write-ups, this is a reasonable template to
start from.
