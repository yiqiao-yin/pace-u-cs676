...
- **The live Claude paths have never been executed** — `credibility.llm_opinion`, `main.ask_claude`, and `ClaudeLLM.complete` are written against the SDK but were never run against the live API. Flagged at each call site and in both project READMEs, worded as "shipped unverified" so it stays true once someone does run it. Do not describe them as tested. Note `deliverable/project_1/.env` holds a real key — **do not spend it without asking.**
