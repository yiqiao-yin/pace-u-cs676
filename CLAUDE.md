...
- **Hugging Face removed the Streamlit SDK.** The picker offers only Gradio, Docker, and static HTML, so a Streamlit app deploys under **Docker** — `sdk: docker`, `app_port: 7860`, and `--server.address=0.0.0.0` in the `CMD`, or it builds and then times out. Both project READMEs carry the working Dockerfile. **Verified 2026-09-25** against a live student Space (`sdk: docker`, `hardware: cpu-basic`, `RUNNING`, created that week) — so the bonus needs **no paid plan**, despite HF's own docs page claiming Docker Spaces require one. Trust the API (`https://huggingface.co/api/spaces/<owner>/<name>`) over the docs page here; that mismatch already produced one wrong answer to a student.
